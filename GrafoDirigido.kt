...
// nombre del archivo: GrafoDirigido.kt
...
public class GrafoDirigido : Grafo {
    ...
    // grafo: es un array de listas mutables en el cual sus elementos son Arcos y su valor inivial es un array vacio
    var grafo : Array<MutableList<Arco>> = arrayOf()

    // Se construye un grafo a partir del numero de vertices
    constructor(numDeVertices: Int) {
        this.grafo = Array(numDeVertices){mutableListOf()}
    }
    ...
    constructor(nombreArchivo: String)  {
        ...
        //slice retorna los elementos de una colleccion especificados por sus indices
        ...
        for (linea in lineas.slice(2..(TotaldeLineas - 1))){
            ...
            if (separarArcos.size < 2){
                throw RuntimeException("Error: input empty")
            }
            //construimos las variables incio y fin para almacenar los nodos
            val inicio = separarArcos[0].toInt()
            val final = separarArcos[1].toInt()

            this.agregarArco(Arco(inicio,final))
        }
    }
    ...
    PRECONDICIÓN: ((x in 0..this.grafo.size-1) && (y in 0..grafo.size-1))
    ...
    */
    fun agregarArco(a: Arco) : Boolean {
        ...
        //verificamos si el lado existe en el grafo y retornamos true
    ...
    POSTCONDICIÓN: (grado >= 0 && grado(v) == gradoExterior(v)+gradoInterior(v))
    ...
    */
    ...
    POSTCONDICIÓN: (gradoExterior = this.grafo[v].size)
    ...
    */
    ...
    fun gradoExterior(v: Int) : Int {
        ...
        var exterior: Int = this.grafo[v].size
    ...
    */
    ...
    PRECONDICIÓN: (v in 0..rhis.grafo.size-1)
    ...
    */
    ...
    /* Retorna los lados adyacentes de un lado l. 
     Dos lados son adyacentes si tinen un extremo en comun.
    ...
    fun ladosAdyacentes(l: Arco) : Iterable<Arco> {
        ...
        var ladosAdy: MutableList<Arco> = mutableListOf()
    ...
    override operator fun iterator() : Iterator<Arco> {
        ...
        var listArco = listOf<Arco>()

        for (t in this.grafo){
            listArco+=(t)
        }

        return listArco.iterator()
    }
